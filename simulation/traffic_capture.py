...
class NetworkSimulator:
    """Simulates network effects on traffic."""

    def __init__(
        self,
        base_delay: float = 0.05,  # 50ms base delay
        jitter: float = 0.01,      # 10ms jitter
        loss_rate: float = 0.01,   # 1% packet loss
        bandwidth_mbps: float = 100  # 100 Mbps bandwidth
    ):
        """
    ...
    def apply_network_effects(
        ...
        """
        Apply network effects to packets and record them.

        Args:
            packets: List of packet dictionaries
            capture: TrafficCapture instance to record packets
        ...
        for packet in packets:
            ...
            delay = self.base_delay + np.random.uniform(-self.jitter, self.jitter)
            delay = max(0, delay)

            # Calculate transmission delay based on packet size
            packet_size_bits = packet.get('size', 1000) * 8
            transmission_delay = packet_size_bits / (self.bandwidth_mbps * 1e6)

            # Total delay
            total_delay = delay + transmission_delay

            # Record packet with network effects
            capture.record_packet_from_network(
                original_time=packet['time'],
                direction=packet['direction'],
                size=packet.get('size', 1000),
            ...
            processed_packet = {
                **packet,
                'network_delay': total_delay,
                'adjusted_time': packet['time'] + total_delay
            }
